...
import { Button, Card, Checkbox, Col, Image, InputNumber, Row, Space, Table, Typography } from "antd"
...
export default function CartInfo() {
  ...
  const handleQuantityChange = (value: number | null, record: CartItemWithKey) => {
    ...
    setLocalItems(prev => prev.map(i => (i.key === record.key ? { ...i, quantity: value } : i)))
  }
  ...
  const columns = [
    {
      ...
      render: (_: boolean, record: CartItemWithKey) => (
        <Checkbox checked={record.selected} onChange={e => handleSelectItem(e.target.checked, record)} />
      ),
      width: 180,
    },
    {
    ...
    },
    {
      ...
      render: (_: unknown, record: CartItemWithKey) => (
        <Button type="text" icon={<FaTrashCan />} onClick={() => handleRemove(record)} className="!text-[#657384]" />
      ),
    },
  ]
  ...
  return localItems.length > 0 ? (
    <>
      <Link href={"/shop"} className="px-10 flex gap-1 items-center text-[#1250dc] font-medium hover:text-[#5979c4]">
    ...
    <Card className="!mx-10 !mb-10 !mt-2 !flex !justify-center !items-center">
      <Text className="!text-center !text-base !text-[#4a4f63] !font-semibold">{t("emptyCart")}</Text>
    </Card>
  )
}
