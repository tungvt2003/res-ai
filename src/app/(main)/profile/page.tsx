...
export default function PatientProfile() {
  ...
  const { data: appointmentsData, isLoading: isLoadingAppointments } = useGetAppointmentsByPatientId(
    patientId || undefined,
  )

  // Fetch orders from API
  const { data: ordersData, isLoading: isLoadingOrders } = useGetOrdersByPatientId(patientId || undefined)
      ...
      <Layout style={{ minHeight: "90vh" }} className="px-10 pt-4 flex flex-row gap-4 min-h-screen">
        ...
        <Layout>
          <Content className="p-6 bg-white rounded-2xl">
            ...
            {selectedKey === "invoice" && <InvoiceSection orders={orders} loading={isLoadingOrders} />}
