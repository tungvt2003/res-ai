...
const SelectSpecialtyStep = ({ doctor, onNext }: { doctor: Doctor; onNext: () => void }) => {
  ...
  const handleNext = () => {
    ...
    localStorage.setItem("bookingService", JSON.stringify(services?.find(s => s.name === selectedService)))
    onNext()
  }
...
const SelectDateTimeStep = ({ doctor, onNext, onBack }: { doctor: Doctor; onNext: () => void; onBack: () => void }) => {
  ...
  const availableDays = monthData?.data?.map(slot => dayjs(slot.start_time).format("YYYY-MM-DD")) || []
              ...
              rules={[
                { required: true, message: "Vui lòng chọn ngày sinh" },
                {
                  validator: (_, value) => {
                    ...
                    return age > 0 && age <= 100 ? Promise.resolve() : Promise.reject("Tuổi phải từ 1 đến 100")
                  },
                },
              ]}
            >
...
export default function BookAppointmentPage() {
  ...
  return (
    <div style={{ padding: "24px", maxWidth: "900px", margin: "auto" }}>
      ...
      {currentStep === 1 && <SelectSpecialtyStep doctor={doctor} onNext={() => setCurrentStep(2)} />}
      {currentStep === 2 && (
        <SelectDateTimeStep doctor={doctor} onNext={() => setCurrentStep(3)} onBack={() => setCurrentStep(1)} />
      )}
