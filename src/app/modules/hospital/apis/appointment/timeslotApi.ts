...
class TimeSlotClient {
  ...
  async getByDoctorAndDate(doctorId: string, date: string): Promise<ListTimeSlotsByDateResponse> {
    const response = await this.client.get<ListTimeSlotsByDateResponse>(`${endpoint}/doctor/${doctorId}/date`, {
  ...
  async getByDoctorAndMonth(doctorId: string, month: string): Promise<ListTimeSlotsByMonthResponse> {
    const response = await this.client.get<ListTimeSlotsByMonthResponse>(`${endpoint}/doctor/${doctorId}/month`, {
