...
class HospitalClient {
  ...
  async update(hospitalId: string, updateData: FormData): Promise<UpdateHospitalResponse> {
    const response = await this.client.put<UpdateHospitalResponse>(`${endpoint}/${hospitalId}`, updateData, {
      headers: { "Content-Type": "multipart/form-data" },
    })
    return response.data
  }
  ...
  async findNearby(latitude: number, longitude: number, radius_km: number): Promise<ListNearbyHospitalsResponse> {
