...
class MedicalRecordClient {
  ...
  async getByPatientId(patientId: string): Promise<GetMedicalRecordByPatientIdResponse> {
    const response = await this.client.get<GetMedicalRecordByPatientIdResponse>(`${endpoint}/patient`, {
  ...
  async initRecordAndDiagnosis(
    ...
    const response = await this.client.post<InitMedicalRecordAndDiagnosisResponse>(`${endpoint}/init`, payload)
    return response.data
  }
}
